---
theme: default
background: https://source.unsplash.com/collection/94734566/1920x1080
class: text-center
highlighter: shiki
lineNumbers: false
info: |
  ## Architecture MLOps - Jinsudai
  Présentation de l'architecture complète de prédiction énergétique
drawings:
  persist: false
transition: slide-left
title: Architecture MLOps
mdc: true
---

# Architecture MLOps
## Jinsudai - Prédiction Énergétique

<div class="pt-12">
  <span @click="$slidev.nav.next" class="px-2 py-1 rounded cursor-pointer" hover="bg-white bg-opacity-10">
    Appuyez sur <kbd>Space</kbd> pour la page suivante
  </span>
</div>

<div class="abs-br m-6 flex gap-2">
  <a href="https://github.com/slidevjs/slidev" target="_blank" alt="GitHub"
    class="text-xl icon-btn opacity-50 !border-none !hover:text-white">
    <carbon:logo-github />
  </a>
</div>

---
layout: two-cols
---

# Objectifs

<template v-slot:default>

- **Création d'algorithmes IA** adaptés aux données d'entraînement
- **Adaptation de l'infrastructure** API pour production
- **Conception de pipelines CI/CD** automatisés
- **Développement de scripts** réentraînement auto
- **Pilotage de la performance** monitoring production

</template>

<template v-slot:right>

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Algorithmes IA] --> B[Infrastructure API]
    B --> C[Pipelines CI/CD]
    C --> D[Scripts Retraining]
    D --> E[Monitoring Production]
    
    style A fill:#4a90d9
    style B fill:#8b9dc3
    style C fill:#1e3a5f
    style D fill:#2d4a6f
    style E fill:#3d5a7f
```

</template>

---
layout: center
class: text-center
---

# Architecture Globale

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Données BRUT<br/>CSV PRM] --> D[Airflow]
    B[Données Météo<br/>API Open-Meteo] --> D
    C[Données Vacances<br/>API] --> D
    D --> F[MLflow<br/>Model Registry]
    D --> G[AutoGluon<br/>Training]
    F --> H[FastAPI<br/>REST API]
    F --> I[Streamlit<br/>UI Prédictions]
    H --> L[PostgreSQL<br/>Prédictions]
    D --> J[Evidently AI<br/>Drift Detection]
    J --> K[Grafana<br/>Dashboards]
    F --> M[S3<br/>Artefacts MLflow]
    L --> J
```

---
layout: center
class: text-center
---

# Services Dockerisés

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    ML1[MLflow Server<br/>Port 7860] --> FA1[FastAPI Service<br/>Port 8000]
    FA1 --> EV1[Evidently AI<br/>Port 8501]
    EV1 --> GR1[Grafana<br/>Port 3000]
    GR1 --> AF1[Airflow Webserver<br/>Port 8080]
    
    style ML1 fill:#ff6b6b
    style FA1 fill:#4ecdc4
    style EV1 fill:#45b7d1
    style GR1 fill:#f39c12
    style AF1 fill:#e74c3c
```

---
layout: center
class: text-center
---

# Pipeline d'Ingestion

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Données BRUT<br/>CSV PRM] --> D[Airflow]
    B[Données Météo<br/>API Open-Meteo] --> D
    C[Données Vacances<br/>API] --> D
    D --> E[Validation]
    E --> F[Stockage<br/>PostgreSQL]
    
    style A fill:#1e3a5f
    style B fill:#2d4a6f
    style C fill:#3d5a7f
    style D fill:#4a90d9
    style F fill:#8b9dc3
```

---
layout: center
class: text-center
---

# Pipeline d'Entraînement

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A1[setup_training_task] --> A2[load_data_task]
    A2 --> A3[validate_data_task]
    A3 --> A4[prepare_data_task]
    A4 --> A5[train_model_task]
    A5 --> A6[evaluate_model_task]
    A6 --> A7[log_to_mlflow_task]
    A7 --> A8[promote_to_prod_task]
    
    style A1 fill:#1e3a5f
    style A5 fill:#4a90d9
    style A7 fill:#8b9dc3
```

---
layout: center
class: text-center
---

# Flux d'Entraînement Complet

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Données brutes CSV] --> B[Data Validation]
    B --> C[Data Preparation]
    C --> D[Feature Engineering]
    D --> E[Split Train/Test<br/>80/20]
    E --> F[Training AutoGluon]
    F --> G[Évaluation Modèle]
    G --> H{Performance OK?}
    H -->|Oui| I[Log MLflow]
    H -->|Non| J[Hyperparameter Tuning]
    J --> F
    I --> K[Promotion Production]
    K --> L[Upload S3<br/>Données entraînées]
    
    style B fill:#1e3a5f
    style F fill:#4a90d9
    style G fill:#8b9dc3
    style H fill:#ff6b6b
    style I fill:#4ecdc4
    style L fill:#ffcccb
```

---
layout: two-cols
---

# Pipeline d'Entraînement - Étapes

<template v-slot:default>

1. **Chargement des données**
   - Téléchargement depuis S3 si manquant
   - Chargement depuis data/raw/

2. **Validation des données**
   - Vérification schéma
   - Rapport Evidently

3. **Transformation**
   - Nettoyage
   - Feature engineering

4. **Préparation**
   - Split train/test
   - Prétraitement avancé

</template>

<template v-slot:right>

5. **Entraînement**
   - AutoGluon / RandomForest
   - Hyperparameter tuning

6. **Évaluation**
   - Métriques (R², MAE, RMSE)
   - Monitoring performance

7. **MLflow Logging**
   - Log métriques
   - Sauvegarde modèle

8. **Gestion Stages**
   - Promotion Staging → Production
   - Validation métriques

9. **Upload S3**
   - Données entraînées
   - Archivage automatique

</template>

---
layout: center
class: text-center
---

# Pipeline d'Inférence

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[setup_prediction_task] --> B[load_model_task]
    B --> C[generate_inference_data_task]
    C --> D[prepare_features_task]
    D --> E[run_predictions_task]
    E --> F[store_predictions_task]
    F --> G[detect_drift_task]
    G --> H{Drift détecté?}
    H -->|Oui| I[retrain_model_task]
    H -->|Non| J[verify_results_task]
    I --> J
    
    style A fill:#1e3a5f
    style E fill:#4a90d9
    style G fill:#ff6b6b
    style I fill:#ffcccb
    style J fill:#4ecdc4
```

---
layout: two-cols
---

# Base de Données - PostgreSQL

<template v-slot:default>

## Table `consumption_predictions`

| Colonne | Type | Description |
|---------|------|-------------|
| `prediction_id` | UUID | Identifiant unique (clé primaire) |
| `target_timestamp` | TIMESTAMP | Timestamp de la prédiction |
| `prediction` | DOUBLE | Valeur prédite en kWh |
| `model_version` | TEXT | Version du modèle |
| `entity_id` | TEXT | Identifiant entité (client/site) |
| `run_id` | TEXT | ID du run MLflow |
| `actual_value` | DOUBLE | Valeur réelle observée |

**Index** : timestamp, entity_id, run_id

</template>

<template v-slot:right>

## Classe `DatabaseHandler`

- `create_tables()` : Création table + index
- `store_predictions()` : Stockage des prédictions
- `get_recent_predictions()` : Récupération récentes
- `get_predictions_by_date()` : Récupération par plage
- `update_actual_values()` : Mise à jour valeurs réelles
- `get_production_data_for_retraining()` : Données pour retraining

**Validation** :
- Espacement 30 minutes entre prédictions
- Tri par timestamp décroissant

</template>

---
layout: center
class: text-center
---

# Pipeline CI/CD

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Push sur main] --> B[GitHub Actions Trigger]
    B --> C[Linting & Tests]
    C --> D{Tests OK?}
    D -->|Non| E[Échec CI]
    D -->|Oui| F[Build Docker Images]
    F --> G[Push Registry]
    G --> H[Deploy MLflow]
    H --> I[Deploy FastAPI]
    I --> J[Deploy Evidently]
    J --> K[Deploy Grafana]
    K --> L[Integration Tests]
    L --> M{Integration OK?}
    M -->|Non| E
    M -->|Oui| N[Deploy Production]
    N --> O[Health Checks]
    O --> P[Notification Success]
    
    style C fill:#1e3a5f
    style F fill:#4a90d9
    style M fill:#ff6b6b
    style O fill:#4ecdc4
```

---
layout: center
class: text-center
---

# Monitoring et Drift Detection

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[FastAPI<br/>Prédictions] --> B[PostgreSQL<br/>Historique]
    B --> C[Evidently AI<br/>Drift Detection]
    C --> D[Grafana<br/>Dashboards]
    C --> E[MLflow<br/>Métriques]
    C --> F[Email Resend<br/>Notifications]
    D --> G[Slack/Webhook<br/>Alertes]
    
    style C fill:#ff6b6b
    style D fill:#4a90d9
    style E fill:#1e3a5f
    style F fill:#ffcccb
```

---
layout: center
class: text-center
---

# Pipeline de Détection Drift

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A[Données Production] --> B[Chargement Référence<br/>S3]
    B --> C[Comparaison Distributions]
    C --> D[Comparaison Prédictions]
    D --> E[Calcul PSI]
    E --> F{Drift > Seuil?}
    F -->|Oui| G[Génération Rapport]
    F -->|Non| H[Monitoring Continu]
    G --> I[Sauvegarde MLflow]
    I --> J[Sauvegarde Workspace<br/>Evidently UI]
    J --> K[Sauvegarde S3]
    K --> L[Alerte Email]
    L --> M[Trigger Retraining]
    M --> N[Nouveau Modèle]
    N --> H
    
    style C fill:#1e3a5f
    style F fill:#ff6b6b
    style G fill:#4a90d9
    style K fill:#ffcccb
    style M fill:#4ecdc4
```

---
layout: two-cols
---

# Monitoring - Étapes

<template v-slot:default>

1. **Chargement données référence**
   - Téléchargement depuis S3
   - Préfixe: consumption/reference

2. **Chargement données courantes**
   - Téléchargement depuis S3
   - Préfixe: consumption/prepared

3. **Détection drift**
   - Data drift (PSI)
   - Concept drift
   - Feature drift

4. **Génération rapport**
   - Rapport Evidently HTML
   - Sauvegarde workspace local
   - Sauvegarde S3

</template>

<template v-slot:right>

5. **Stockage métriques**
   - MLflow tracking
   - Métriques drift

6. **Notifications**
   - Email via Resend
   - Slack webhooks
   - Alertes automatiques

**Types de drift** :
- Data Drift : Distribution features
- Concept Drift : Relation features/target
- Prediction Drift : Distribution prédictions

</template>

---
layout: center
class: text-center
---

# Automatisation du Retraining

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e3a5f', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#4a90d9', 'lineColor': '#8b9dc3', 'secondaryColor': '#2d4a6f', 'tertiaryColor': '#3d5a7f', 'background': '#0f172a', 'mainBkg': '#1e3a5f', 'nodeBorder': '#4a90d9', 'clusterBkg': '#1e293b', 'clusterBorder': '#334155', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#1e3a5f'}}}%%
graph LR
    A1[Drift Détecté] --> D[Trigger Retraining]
    A2[Cycle Hebdo] --> D
    A3[Données Nouvelles] --> D
    B1[API Trigger] --> D
    B2[CLI Command] --> D
    B3[Airflow DAG] --> D
    D --> E{Conditions OK?}
    E -->|Oui| F[Lancement Training]
    E -->|Non| G[Annulation]
    F --> H[Évaluation Nouveau Modèle]
    H --> I{Amélioration?}
    I -->|Oui| J[Promotion Production]
    I -->|Non| K[Conserve Ancien]
    
    style A1 fill:#ffcccb
    style A2 fill:#4a90d9
    style A3 fill:#4a90d9
    style E fill:#ff6b6b
    style J fill:#4ecdc4
```

---
layout: two-cols
---

# Triggers de Retraining

<template v-slot:default>

## Triggers Automatiques

- **Drift Detection**
  - R² < seuil d'alerte
  - Trigger immédiat

- **Cycle Hebdomadaire**
  - 7 jours depuis dernier retraining
  - Si nouvelles données disponibles

- **Nouvelles Données**
  - >= 1000 nouveaux exemples
  - Vérification quotidienne

</template>

<template v-slot:right>

## Triggers Manuels

- **API Trigger**
  - Endpoint: `/retrain`
  - Méthode: POST

- **CLI Command**
  - `python scripts/retrain.py`
  - Options: `--force`, `--dry-run`

- **Airflow Trigger**
  - DAG: `retraining_pipeline`
  - Schedule: Manuel ou cron

## Conditions de Validation

- R² nouveau >= R² courant
- MAE nouveau <= MAE courant
- RMSE nouveau <= RMSE courant

</template>

---
layout: two-cols
---

# Métriques par Domaine

<template v-slot:default>

## Consommation Électrique

- **R² cible**: >= 0.90
- **R² alerte**: < 0.85
- **Métriques**: R², MAE, RMSE
- **Seuil drift**: PSI > 0.1

## Production Solaire

- **R² cible**: >= 0.92
- **R² alerte**: < 0.88
- **Métriques**: R², MAE, RMSE
- **Seuil drift**: PSI > 0.1

</template>

<template v-slot:right>

## Gestion des Stages MLflow

- **Staging** : Modèle entrainé
- **Production** : Modèle promu
- **Validation** : Comparaison métriques
- **Alias** : `prod` pour production

## Intégration S3

- **Artefacts** : Modèles MLflow
- **Données** : Train parquets
- **Rapports** : Evidently HTML
- **Archivage** : Automatique

</template>

---
layout: center
class: text-center
---

# Spécifications Techniques

| Spécification | Implémentation | Statut |
|---------------|----------------|--------|
| Algorithmes IA | AutoGluon (regression) | ✅ |
| Métriques | R² >= 0.90 / 0.92 | ✅ |
| Temps inférence | < 100ms | ✅ |
| API Production | FastAPI endpoints | ✅ |
| CI/CD | GitHub Actions + Docker | ✅ |
| Réentraînement auto | Airflow DAGs | ✅ |
| Monitoring | Evidently AI + Grafana | ✅ |
| Alertes | Email + Slack webhooks | ✅ |
| Stockage S3 | Artefacts + Données | ✅ |
| Gestion Stages | MLflow Registry | ✅ |

---
layout: center
class: text-center
---

# Conclusion

Cette architecture MLOps complète respecte l'intégralité du cahier des charges :

<div class="grid grid-cols-2 gap-4 pt-8">

<div>

1. **Algorithmes IA adaptés**
   - AutoGluon avec configurations spécifiques
   - Support multi-domaines

2. **Infrastructure adaptée**
   - API FastAPI + UI Streamlit
   - Services dockerisés

3. **Pipelines CI/CD**
   - GitHub Actions automatisés
   - Déploiement Hugging Face Spaces

</div>

<div>

4. **Scripts réentraînement**
   - Airflow DAGs avec triggers automatiques
   - Gestion des stages MLflow
   - Intégration S3

5. **Pilotage performance**
   - Evidently AI + Grafana
   - Détection drift avancée
   - Notifications automatiques

</div>

</div>

Architecture modulaire, scalable et conforme aux meilleures pratiques MLOps.

---
layout: center
class: text-center
---

# Merci

<div class="pt-12">
  <span class="text-4xl font-bold">Questions ?</span>
</div>

<div class="pt-8">
  <span class="text-xl opacity-75">
    Présentation générée avec Slidev - Architecture MLOps Jinsudai
  </span>
</div>
