"""
Airflow DAG pour déclencher le pipeline de retraining via GitHub Action.

Ce DAG:
- Déclenche le workflow GitHub Action 6_retraining-pipeline.yml
...
with DAG(
    ...
    description='Déclenche le pipeline de retraining via GitHub Action',
    schedule=None,
    catchup=False,
    tags=['retraining', 'github-action'],
) as dag:
    
    trigger_task = PythonOperator(
        task_id='trigger_github_action',
        python_callable=trigger_github_action,
        op_kwargs={'github_workflow': '6_retraining-pipeline.yml'},
    )
