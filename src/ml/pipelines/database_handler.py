...
class DatabaseHandler:
    ...
    def get_predictions_for_drift_detection(self, limit=100, start_date=None, end_date=None):
        ...
        Args:
            ...
            start_date: Date de début (datetime ou string, optionnel)
            end_date: Date de fin (datetime ou string, optionnel)
        Returns:
            DataFrame avec target_timestamp, prediction, actual_value
        """
        if not self.db_uri:
            logger.warning("DB URI non fournie, récupération ignorée")
            return None
        ...
        try:
            ...
            logger.info(f"Données pour drift detection récupérées: {len(df)} enregistrements")
            return df
        except Exception as e:
